...
const ItemsMenuContainer = (data:ItemDataInterface) => {
    ...
    const {getImage, url} = useFirebaseImage();
    const setItemsImageURL=(path:string)=>{
        getImage(path);
        PERSON_ITEMS[0].img = url;
        console.log("==>", PERSON_ITEMS[0])
    }
    setItemsImageURL('/itemsImg'+PERSON_ITEMS[0].ima_tag);
